 ...
 * plugins/xvideos.js
 *
 * Download an Xvideos/Xvideos2 video page URL:
 *  - Usage: .xvideos <xvideos_video_url>
 *
 * Notes:
 *  - Supports both xvideos.com and xvideos2.com variants (with or without www).
 *  - Extracts progressive MP4s via heuristics, probes sizes, prefers non-preview files,
 *    downloads via streaming (no child_process), and sends as document.
 *
...
// Helper: derive a safe referer/origin from a URL string
...
async function probeSizeMB(url) {
  try {
    const referer = getOrigin(url);
    const res = await axiosClient.head(url, { maxRedirects: 5, timeout: 15000, headers: { Referer: referer } });
...
async function extractGenericVideoCandidates(pageUrl) {
  try {
    const res = await axiosClient.get(pageUrl, { headers: { Referer: getOrigin(pageUrl) } });
    const html = res.data || "";
    ...
    // collect scripts but limit size to avoid heavy parse
    const scripts = [];
    $("script").each((i, s) => {
      const t = $(s).html();
...
cmd(
  {
    pattern: "xvideos",
    react: "⏬",
    desc: "Download video from xvideos/xvideos2 (provide a page URL).",
    category: "download",
    filename: __filename,
  },
  async (robin, mek, m, { from, q, reply }) => {
    try {
      if (!q) return reply("Provide an xvideos/xvideos2 video URL. Example: .xvideos <url>");
      const url = q.trim();
      if (!/^https?:\/\//i.test(url)) return reply("Provide a valid http/https URL.");
      // Accept xvideos.com and xvideos2.com (with or without www)
      if (!/xvideos(2)?\.com/i.test(url)) return reply("That URL does not look like an xvideos or xvideos2 page.");

      await robin.sendMessage(from, { text: `🔎 Processing xvideos URL...\n${url}\nPlease wait...` }, { quoted: mek });
      ...
      console.error("xvideos handler error:", e);
